...
const whereToAddObject = {
    head:        (documentElement, s) => {documentElement.head.prepend(s)},
    body_top:    (documentElement, s) => {documentElement.body.prepend(s)},
    body_bottom: (documentElement, s) => {documentElement.body.appendChild(s)}
}
